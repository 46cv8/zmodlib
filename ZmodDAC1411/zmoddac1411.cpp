 ...
 *  platform, and is irrelevant on a Linux platform.
 */
ZMODDAC1411::ZMODDAC1411(uintptr_t baseAddress, uintptr_t dmaCh1Address, uintptr_t dmaCh2Address, uintptr_t iicAddress, uintptr_t flashAddress, int dmaCh1Interrupt, int dmaCh2Interrupt)
		: ZMOD(baseAddress, dmaCh1Address, dmaCh2Address, iicAddress, flashAddress, DMA_DIRECTION_TX, -1, dmaCh1Interrupt, dmaCh2Interrupt)
{
...
/**
* Allocates the data buffer used for AXI DMA transfers, 2 bytes for each sample.
* The length of the allocated buffer is limited to the maximum supported buffer length,
...
*
*/
uint16_t* ZMODDAC1411::allocBuffer(uint8_t channel, size_t &length) {
	if(length > ZmodDAC1411_MAX_BUFFER_LEN)
	{
		length = ZmodDAC1411_MAX_BUFFER_LEN;
	}
	return (uint16_t *)ZMOD::allocDMABuffer(channel, length * sizeof(uint16_t));
}

/**
* Free the data buffer used for AXI DMA transfers, 2 bytes for each sample.
*
* @param channel the channel: 0 for channel 1, 1 for channel 2
...
*
*/
void ZMODDAC1411::freeBuffer(uint8_t channel, uint16_t *buf, size_t length) {
	ZMOD::freeDMABuffer(channel, buf, length * sizeof(uint16_t));
}
...
* The length of the data buffer is limited to the maximum supported buffer length,
* altering the value of the reference parameter accordingly.
* *
* @param channel the channel: 0 for channel 1, 1 for channel 2
...
*/
uint8_t ZMODDAC1411::setData(uint8_t channel, uint16_t* buffer, size_t &length)
{
	uint8_t Status;
	if(length > ZmodDAC1411_MAX_BUFFER_LEN)
	{
	...
	// Start DMA Transfer
	Status = startDMATransfer(channel, buffer, length * sizeof(uint16_t));
	if (Status) {
		return ERR_FAIL;
	}
	return Status;
 ...
 */
uint16_t ZMODDAC1411::getSignedRawFromVolt(float voltValue, uint8_t gain)
{
	...
	return ((uint16_t)((uint16_t)raw & 0x3FFF)) << 2;
}
